...
--   - evidencia_documento / evidencia_control: los documentos de respaldo de
--     la auditoría, en relación muchos a muchos con evaluacion_control (un
--     documento puede sustentar varios controles; un control puede tener
--     varios documentos). evidencia_documento.id_auditoria vive duplicado
--     ahí, no solo deducible por join, porque "vincular un documento ya
--     cargado" filtra por auditoría todo el tiempo.
...
-- Documentos de respaldo: evidencia_documento guarda el documento en sí;
-- evidencia_control solo enlaza (muchos a muchos con evaluacion_control).
CREATE TABLE evidencia_documento (
                                     id_evidencia       NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                                     id_auditoria       NUMBER        NOT NULL,
                                     nombre_documento   VARCHAR2(200) NOT NULL,
                                     ...
                                     CONSTRAINT fk_evidencia_auditoria
                                         FOREIGN KEY (id_auditoria) REFERENCES auditoria (id_auditoria),
                                     CONSTRAINT ck_evidencia_formato
                                         CHECK (formato IN ('PDF', 'DOCX', 'XLSX', 'PNG', 'JPG', 'CSV', 'TXT', 'LOG'))
);

CREATE INDEX ix_evidencia_auditoria ON evidencia_documento (id_auditoria);

CREATE TABLE evidencia_control (
    -- id_evaluacion_control va primero en la PK: la lectura más frecuente es
    -- "los documentos de ESTE control", y ese orden es el que el índice de
    -- la propia llave primaria deja resuelto sin columna adicional.
                                   id_evaluacion_control   NUMBER NOT NULL,
                                   id_evidencia            NUMBER NOT NULL,
                                   CONSTRAINT pk_evidencia_control
                                       PRIMARY KEY (id_evaluacion_control, id_evidencia),
                                   CONSTRAINT fk_evctrl_evaluacion
                                       FOREIGN KEY (id_evaluacion_control) REFERENCES evaluacion_control (id_evaluacion_control),
                                   CONSTRAINT fk_evctrl_evidencia
