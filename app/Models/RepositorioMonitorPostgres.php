...
/**
 * Monitor de salud sobre PostgreSQL: las dos caras, lectura y escritura.
 *
 * Ruta ALTERNATIVA a RepositorioMonitorOracle. Mismo par de contratos
 * (RepositorioMonitor / RepositorioMonitorEscritura), SQL adaptado.
 *
 * ADVERTENCIA (ver CLAUDE.md del proyecto): a día de hoy /monitoreo es una
 * MAQUETA — la vista lee config/monitor-mockup.php, no este repositorio, sin
 * importar el motor de base de datos. Esta clase existe para que el día que
 * el frente 4 conecte la fuente real, la mitad Postgres esté tan lista como
 * la mitad Oracle, no porque algo la esté llamando hoy.
 *
 * Convenciones traducidas de la versión Oracle:
 *
 * - Los instantes siguen viajando como texto ISO 8601 en UTC
 *   ('2026-08-19T14:35:02', con o sin sufijo de zona — sinZonaHoraria() lo
 *   recorta). La columna es TIMESTAMP sin zona en los dos motores.
 *
 * - Las series y la retención salen de las funciones consolidar_hora()/
 *   purgar() (Scripts/postgres/09_procedimientos_monitor.sql), nunca de un
 *   DELETE o una agregación sueltos en PHP — misma regla que en Oracle.
 *
 * - guardarMuestra(), agruparEnEpisodio() y versionarUmbral() siguen siendo
 *   las únicas escrituras de más de una sentencia, y las tres corren dentro
 *   de una transacción (BaseDatosPostgres::iniciarTransaccion()).
 *
 * Diferencias de dialecto frente a la versión Oracle:
 *
 * - TO_CHAR(ts, 'YYYY-MM-DD"T"HH24:MI:SS') -> TO_CHAR(ts, 'YYYY-MM-DD"T"HH24:MI:SS')
 *   es la MISMA función y el MISMO patrón en Postgres, así que no cambia.
 * - TO_TIMESTAMP(texto, patron) -> igual, misma función en los dos motores.
 * - LISTAGG(...) WITHIN GROUP (ORDER BY ...) -> STRING_AGG(... ORDER BY ...),
 *   el equivalente de Postgres (no tiene LISTAGG).
 * - FETCH FIRST n ROWS ONLY -> igual, Postgres lo soporta con la misma
 *   sintaxis SQL:2008.
 * - RETURNING ... INTO :id (Oracle) -> el propio RETURNING de Postgres, leído
 *   por BaseDatosPostgres::insertar() con fetchColumn(); no hace falta un
 *   bind de salida aparte.
 * - SYSTIMESTAMP -> now().
 * - BEGIN pkg_monitor.consolidar_hora(...); END; -> SELECT consolidar_hora(...)
 *   (procedimiento() ejecuta cualquier sentencia; aquí es un SELECT a una
 *   función, no un CALL, porque consolidar_hora()/purgar() se declararon
 *   como FUNCTION que RETURNS void).
 */
final class RepositorioMonitorPostgres implements RepositorioMonitor, RepositorioMonitorEscritura
{
     ...
     * La causa sale de una subconsulta con STRING_AGG porque es una lista de
    ...
    private const SELECCION_MUESTRA = <<<'SQL'
        SELECT mu.id_muestra, mu.clave_instancia,
               ...
               (SELECT STRING_AGG(ic.codigo_metrica, ',' ORDER BY ic.codigo_metrica)
    ...
    public function serieIndice(string $clave, string $desdeUtc, string $hastaUtc): array
    {
        return $this->bd->consultar(
            "SELECT TO_CHAR(mu.tomada_en, '" . self::FORMATO . "') AS tomada_en, i.isbd, i.estado
              ...
              WHERE mu.clave_instancia = :clave
                AND mu.tomada_en BETWEEN TO_TIMESTAMP(:desde, '" . self::FORMATO . "')::TIMESTAMP
                                     AND TO_TIMESTAMP(:hasta, '" . self::FORMATO . "')::TIMESTAMP
    ...
    /**
     * Por encima de los 30 días de retención en crudo, la ventana pedida se
     * sirve desde resumen_hora en vez de medicion. Se agrega sobre
     * valor_crudo, no el normalizado.
     *
     * @return list<array<string, mixed>>
     */
               ...
               FROM (
                   SELECT mu.tomada_en AS instante, me.valor_crudo AS valor
                    ...
                    WHERE mu.clave_instancia = :clave
                      ...
                      AND mu.tomada_en >= now() - INTERVAL '30 days'
                    UNION ALL
                    SELECT rh.hora AS instante, rh.promedio AS valor
                     ...
                     WHERE rh.clave_instancia = :clave
                       ...
                       AND rh.hora < now() - INTERVAL '30 days'
               ) serie
              WHERE instante BETWEEN TO_TIMESTAMP(:desde, '" . self::FORMATO . "')::TIMESTAMP
                                  AND TO_TIMESTAMP(:hasta, '" . self::FORMATO . "')::TIMESTAMP
              ORDER BY instante",
            [
                'clave'  => $clave,
        ...
        $filas = $this->bd->consultar(
            "SELECT id_alerta, clave_instancia, codigo_metrica, nivel_actual, nivel_maximo,
              ...
              WHERE estado_atencion = 'ABIERTA'
                AND (:clave::VARCHAR(50) IS NULL OR clave_instancia = :clave)
        ...
        $filas = $this->bd->consultar(
            "SELECT id_episodio, clave_instancia, id_alerta_causa,
              ...
              WHERE clave_instancia = :clave
                AND abierto_en BETWEEN TO_TIMESTAMP(:desde, '" . self::FORMATO . "')::TIMESTAMP
                                   AND TO_TIMESTAMP(:hasta, '" . self::FORMATO . "')::TIMESTAMP
        ...
        $filas = $this->bd->consultar(
            "SELECT me.valor_normalizado
              ...
              WHERE mu.clave_instancia = :clave
                ...
                AND mu.tomada_en >= now() - (:dias || ' days')::interval
    ...
    {
        // $columna sale de una lista fija de dos literales internos
        // ('valor_acumulado' | 'huella'), nunca de entrada del usuario, así
        // que interpolarla en el SQL es seguro — igual que en la versión
        // Oracle.
        $filas = $this->bd->consultar(
        ...
        try {
            $idMuestra = $this->bd->insertar(
                "INSERT INTO muestra (clave_instancia, tomada_en, duracion_ms, resultado, cobertura_pct, mensaje)
                 VALUES (:clave, TO_TIMESTAMP(:tomada_en, '" . self::FORMATO . "')::TIMESTAMP, :duracion_ms, :resultado, :cobertura_pct, :mensaje)
    ...
    public function registrarAlerta(
        ...
        if ($abierta !== null) {
            ...
            // :nivel se castea EXPLÍCITAMENTE la primera vez que aparece.
            // Con ATTR_EMULATE_PREPARES en false (preparación del lado del
            // servidor), Postgres infiere el tipo de cada parámetro por su
            // primer uso en la sentencia y exige que todas las apariciones
            // del MISMO marcador concuerden; sin el cast, "nivel_actual =
            // :nivel" lo infiere como VARCHAR(12) (por la columna) mientras
            // que "CASE :nivel WHEN 'CRITICO' ..." lo infiere como TEXT (por
            // el literal), y la mezcla revienta con "inconsistent types
            // deduced for parameter" — un problema que no existe con
            // Oracle/oci8, donde los binds no fuerzan tipo por contexto.
            $this->bd->ejecutar(
                "UPDATE alerta
                    SET nivel_actual = :nivel::VARCHAR(12),
                        nivel_maximo = CASE
                        ...
                        vista_por_ultima_vez = TO_TIMESTAMP(:vista, '" . self::FORMATO . "')::TIMESTAMP
                  WHERE id_alerta = :id",
                [
        ...
        return $this->bd->insertar(
            "INSERT INTO alerta
             ...
             VALUES (:clave, :codigo, :nivel, :nivel, :valor, :umbral,
                     TO_TIMESTAMP(:vista, '" . self::FORMATO . "')::TIMESTAMP, TO_TIMESTAMP(:vista, '" . self::FORMATO . "')::TIMESTAMP)
             RETURNING id_alerta",
            [
    ...
    public function versionarUmbral(
        ...
        try {
            ...
            // cuatro números, solo sobre valido_hasta.
            $this->bd->ejecutar(
                "UPDATE umbral
                    SET valido_hasta = TO_TIMESTAMP(:desde, '" . self::FORMATO . "')::TIMESTAMP
                  WHERE codigo_metrica = :codigo
                    AND ((:clave::VARCHAR(50) IS NULL AND clave_instancia IS NULL) OR clave_instancia = :clave)
            ...
            $idUmbral = $this->bd->insertar(
                "INSERT INTO umbral (codigo_metrica, clave_instancia, u_opt, u_adv, u_deg, u_crit, valido_desde)
                 VALUES (:codigo, :clave, :u_opt, :u_adv, :u_deg, :u_crit, TO_TIMESTAMP(:desde, '" . self::FORMATO . "')::TIMESTAMP)
                 RETURNING id_umbral",
                [
    ...
    // ── RepositorioMonitorEscritura — Mantenimiento ──────────────────────────

    public function consolidarHora(string $hastaUtc): void
    {
        $this->bd->procedimiento(
            "SELECT consolidar_hora(TO_TIMESTAMP(:hasta, '" . self::FORMATO . "')::TIMESTAMP)",
            ['hasta' => self::sinZonaHoraria($hastaUtc)],
        );
    }
    ...
    {
        $this->bd->procedimiento(
            "SELECT purgar(TO_TIMESTAMP(:hasta, '" . self::FORMATO . "')::TIMESTAMP)",
            ['hasta' => self::sinZonaHoraria($hastaUtc)],
        );
    }
     ...
     * La columna es TIMESTAMP sin zona porque todo el sistema trabaja en UTC.
     */
