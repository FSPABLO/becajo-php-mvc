...
| B-9 | `indice.causa` se separa a una tabla puente `indice_causa` (`id_indice`, `codigo_metrica`), en vez de un campo de texto con códigos separados por coma | Es una lista de métricas en el peor estado (invariante 5), no un escalar; guardarla como texto delimitado obligaría a parsearla tanto en el motor como en la vista cada vez que hace falta consultarla | §7.1 |
| B-10 | Nueva interfaz `MotorCalculo` (un método: `evaluar(muestraCruda, RepositorioMonitor): muestraEvaluada`) para que `bin/monitor.php` invoque al motor de F3 | Ningún contrato de Fase 0 fijó cómo se llaman entre sí el recolector y el motor —los tres contratos cerraron el modelo de datos, no esa costura—. Sin el motor implementado todavía, el agente degrada: recolecta y guarda la muestra como FALLIDA en vez de perder la serie de tiempo o inventar un cálculo | contrato-repositorio-monitor.md (nuevo archivo de contrato) |
| B-11 | `consulta_observada` (evidencia de C-066) viaja intacta de la muestra cruda a la evaluada; el motor no la transforma, solo la reenvía, y `guardarMuestra()` la persiste junto con la cabecera, las mediciones y el índice, en la misma transacción | `v$sqlstats` alimenta a la vez M-CON-01 (el conteo) y el top-N completo (§3.1 del plan); la primera versión de `guardarMuestra()` no cubría el top-N | contrato-muestra.md §3, contrato-repositorio-monitor.md §3 |
